...
```{r}
df <- data |> 
      select ("gender", "amount", "fraud", "category", "age") |>                filter(age!="'U'" & category != c("'es_transportation'",       "'es_food'", "'es_health'"))

```


¿Qué tipo de variable es cada _feature_?
...
```{r}
recipe_df <-  recipe (fraud ~ age+category+ amount, data= df_train) |>  
