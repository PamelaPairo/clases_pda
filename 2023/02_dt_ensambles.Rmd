...
```{r}
data <- read_csv('C:/Users/pamel/Desktop/PDA/ML/data/df_fraude.csv')
glimpse(data)
```
...
Se necesita descargar el paquete `parttree` que no está disponible en CRAN. Ejecutar el siguiente código en la consola.
...
Para más detalles ver [aquí](https://github.com/grantmcdermott/parttree)
...
p = ggplot(data = df_train, aes(x = category_X.es_health., y = amount)) +
    geom_point(aes(col = fraud)) + scale_x_continuous(limits = c(0, 6))+
    theme_minimal()
...
Si tuviese árboles más profundos ¿Cómo se modificaría el gráfico anterior?
